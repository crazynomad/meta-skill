...
> `b067d181-6f89-410c-b2be-7fb1c42bfcfd`(12 篇)
...
| 编译优化 | **DSPy**(斯坦福) | prompt 应该被编译而非手写:声明任务和指标,优化器自动调优。preamble 编译期组合思路的学术终点 |
...
**没有一家标杆覆盖"改进段"** — 使用遥测 → learnings → retro → 蒸馏提案 →
人工裁决的闭环,调研范围内只有 gstack 做了完整实现。
这是最值得借鉴、也最可能被平台化的部分。
